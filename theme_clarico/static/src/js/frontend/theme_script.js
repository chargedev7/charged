...
odoo.define('theme_clarico.theme_script', function(require) {
    ...
    sAnimations.registry.themeSearch = sAnimations.Class.extend({
        ...
        read_events: {
            ...
            'mouseenter .te_dynamic_ept': '_onMouseEnterOpen',
            'mouseleave .te_dynamic_ept': '_onMouseLeaveClose',
        },
        start: function() {
        ...
        _onMouseEnterOpen: function(ev){
            if ($(window).innerWidth() > 1200) {
             var self = ev.currentTarget;
                 $(self).addClass('open');
                 $(self).find(".dropdown-menu").addClass('te_mega_animation');
                 $(self).removeClass('show');
                 $(self).find('.dropdown-menu').removeClass('show');
            }
        },
        _onMouseLeaveClose: function(ev){
            if ($(window).innerWidth() > 1200) {
                var self = ev.currentTarget;
                $(self).removeClass('open');
                $(self).find(".dropdown-menu").removeClass('te_mega_animation');
                $(self).removeClass('show');
                $(self).find('.dropdown-menu').removeClass('show');
            }
        },
        ...
        _onSearchClickOpen: function(ev) {
            ...
            //style7 and style9
            if ($(".te_header_6_srch_icon, .te_header_7_srch_icon").length) {
                if($(window).width() <= 767){
                    if($('.te_header_7_srch_icon').length){
                        $('.navbar-expand-md .navbar-toggler, .te_header_7_srch_icon, header .navbar-brand').hide();
                    }
                    else{
                ...
                if ($(window).width() <= 991) {
                    if($('.te_header_7_srch_icon').length){
                        $(".te_header_7_srch_icon, div#top_menu_collapse .navbar-nav, div#top_menu_collapse_clone .navbar-nav").hide();
                    }
                    else{
    ...
    sAnimations.registry.themeEvent = sAnimations.Class.extend({
        ...
        _onAttribSection: function(ev) {
            ...
            if (main_ul.hasClass("open_ul")) {
                ...
                $(main_ul).parent('.nav-item').find(".te_attr_title i").removeClass('deg90');
                main_ul.toggle('slow');
                ...
                $(main_ul).parent('.nav-item').find(".te_attr_title i").addClass('deg90');
                main_ul.toggle('slow');
...
//        }
    ...
    sAnimations.registry.StickyFilter = sAnimations.Animation.extend({
        selector: '#wrapwrap',
        _stickyFilter: function(ev){
            return true;
        },
    });

    // Prevent search with autocomplete while scrolling header
    sAnimations.registry.scrollToPrevent = sAnimations.Animation.extend({
        selector: '#wrapwrap',
         effects: [{
            startEvents: 'scroll',
            update: '_preventSearchOnScroll',
        }],
        _preventSearchOnScroll: function(scroll){
            /* Prevent search bar while scrolling header */
            if($('.navbar-light.navbar-expand-lg, .navbar-light').length){
                /* odoo default header search */
                if ($("header .te_searchform__popup.open").length) {
                    $('header').css({'transform':'none','transition':'none'});
                }
    ...
    sAnimations.registry.AosAnimate = sAnimations.Animation.extend({
        selector: '#wrapwrap',
        effects: [{
            startEvents: 'scroll',
            update: '_aosRefresh',
